...
and to the server.
For example, WebSockets can be used to create a [chat application](creating_web_socket_chat.md).
...
* Add WebSocket extensions. For example, you can use the [Deflate](websocket_deflate_extension.md) extension or
  implement a [custom extension](websocket_extensions_api.md).

> To learn about WebSocket support on the client-side, see the [WebSockets client plugin](websocket_client.md).
...
the [routing](Routing_in_Ktor.md#define_route) block:
...
a [default configuration](Configuration-file.topic) is used.
...
about [](requests.md#request_information).
...
To handle multiple WebSocket sessions (for example, in a [chat application](creating_web_socket_chat.md)), you need to
