...
> Note that signing/encryption keys shouldn't be specified in code. You can use a custom group in the [configuration file](Configurations.xml#configuration-file-overview) to store signing/encryption keys and initialize them using [environment variables](Configurations.xml#environment-variables).
>
{type="warning"}
