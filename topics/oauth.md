...
[OAuth](https://oauth.net/) is an open standard for access delegation. OAuth can be used to authorize users of your application by using external providers, such as Google, Facebook, Twitter, and access their APIs.

The `oauth` plugin supports the authorization code flow. You can configure OAuth parameters in one place, and Ktor will automatically make a request to a specified authorization server with the necessary parameters.
...
1. A user opens a login page in a Ktor application. `OAuth client ID` credentials
2. Ktor makes an automatic redirect to the authorization page for a specific provider and passes the necessary parameters:
   ...
   * a callback or redirect URL that specifies a Ktor application's page that will be opened after authorization is completed;
...
3. The authorization page suggests a user log in and shows a consent screen with the level of permissions required for a Ktor application. These permissions depend on scopes passed in step 2. 
...
Note that the client also has the [](json.md) plugin installed. This is required to deserialize JSON data about a Google user when [making a request to API](#request-api).
...
* The `client` property specifies the [HttpClient](#create-http-client) used by the server to make requests to the OAuth server.
...
* a token is saved to a cookie [session](sessions.md), whose content can be accessed in other routes;
