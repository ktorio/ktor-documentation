...
2. Switch a branch from `main` to one of the following:
   ...
   ```
   {style="block"}
   These branches demonstrate different ways of [creating and configuring a Ktor server](create_server.xml): in code or by using the `application.conf` configuration file. The only difference in deploying these projects is how to [specify a port](#prepare-app) used to listen for incoming requests.

## Prepare an application {id="prepare-app"}

Perform the following steps to prepare an application for deploying:
