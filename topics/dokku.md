...
[Dokku](https://dokku.com/) is a self-hosted Platform-as-a-Service (PaaS) that runs on your own Linux server and provides a Heroku-like deployment workflow. While you can deploy a Ktor application by copying a fat JAR to a server manually, Dokku automates the surrounding infrastructure:

* **Git-based deploys** — push your code with `git push` and Dokku builds and restarts the application automatically, with no SSH file copying or manual restarts.
* **Process management** — Dokku starts, stops, and restarts your application automatically, including after server reboots.
* **Multiple apps on one server** — each application runs in an isolated container, so there are no port conflicts and apps don't interfere with each other.
* **HTTPS** — a single command provisions a Let's Encrypt certificate for your app.
* **Zero-downtime deploys** — Dokku waits for the new container to pass a health check before switching traffic away from the old one.
...
* If server configuration is specified in code, you can obtain the environment variable value using `System.getenv`. Change the `port` parameter value of the `embeddedServer` function as shown below:
   ```kotlin
   fun main() {
...
* If your server configuration is specified in a configuration file, open `application.conf` or `application.yaml` placed in `src/main/resources` and update the `port` property as shown below:
...
```
Note that the `installDist` task comes with the Gradle [application plugin](https://docs.gradle.org/current/userguide/application_plugin.html).
...
The version must match the JVM toolchain version specified in your `build.gradle.kts`. Without this file, Dokku will use the latest available JDK version, which may change over time and cause unexpected build failures.
