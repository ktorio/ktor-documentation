...
When using the [Routing](Routing_in_Ktor.md) feature, you can access
...
Remember that since HTTP requests and responses are non-seekable streams,
...
* `response.contentRange(1024L until 2048L, 4096L)` - Sets the `Content-Range` header (check the [PartialContent](partial-content.md) feature) 
...
>If you want to make redirections that stop the control flow by throwing an exception, check out this [sample from status pages](status_pages.md).
>
{type="note"}
...
* `call.respond(HttpStatusCode.NotFound, MyDataClass("hello", "world"))` - Specifies a status code, and sends a payload in a single call. Check [StatusPages](status_pages.md)
...
Ktor includes an optional [feature to send HTML content using a DSL](html_dsl.md).

### Sending HTML with FreeMarker
{id="html-freemarker"}

Ktor includes an optional [feature to send HTML content using FreeMarker](freemarker.md).

### Sending JSON with data classes
{id="json"}

Ktor includes an optional [feature to send JSON content using Content negotiation](gson.md).